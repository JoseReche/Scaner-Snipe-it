...
Aplicação para consultar ativos no Snipe-IT/SIpe com autenticação segura baseada em banco local em arquivo (`users.db.json`) e variáveis de ambiente (`.env`), usando API Key pessoal de cada usuário criptografada no backend.
...
- `src/data/users.db.json`: banco local com `matricula`, `password_hash` (bcrypt) e `api_key_encrypted` (AES-256-GCM).
...
- `POST /api/auth/register`
  ...
  - recebe a chave pessoal da API, criptografa no backend e persiste no `users.db.json`
  - persiste hash bcrypt no `users.db.json`
- `POST /api/auth/change-password`
  - rota protegida por JWT
  - valida senha atual
  - gera novo hash bcrypt e grava no `users.db.json`
...
- Há migração automática do formato legado `users.json` para `users.db.json` na primeira execução.
